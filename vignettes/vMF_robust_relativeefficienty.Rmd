...
1/(results[,-1]/results[, "ml"])
```

```{r n100plot}
library(ggplot2)
library(dplyr)
as_tibble(1/(results[,-1]/results[, "ml"]), rownames = "k") %>%
  dplyr::mutate(k = as.numeric(k)) %>%
  tidyr::pivot_longer(-k, names_to = "Estimator", values_to = "relMSE") %>%
  ggplot() +
  geom_line(aes(x = k, y = relMSE, col = Estimator, lty = Estimator)) +
  geom_point(aes(x = k, y = relMSE, col = Estimator)) +
  ggtitle("Relative MSE for Kappa")

mse_m <- t(simplify2array(lapply(n100, function(a){out2mse_m(a$ests, m)})))
