...
ppi_plotdensity <- function(...){
  ...
  data.frame(Yarr, iden) %>%
    ...
    # geom_interpolate_tern(aes(colour = ..level..), base = "identity", bins = 500, base = "identity") +
...
# A Dirichlet with Concentrations on Multiple Edges
...
data.frame(samp) %>%
  ...
  geom_density_tern(bins = 10, bdl = 0.005, base = "identity") + 
  geom_point(size = 1, alpha = 0.5, shape = "+") +
  ggtitle("Sample Points")
```

# A Dirichlet with Concentration on Centre of One Edge
```{r dirichlet2, warning=FALSE, out.width = "0.5\\linewidth"}
ppi_plotdensity(beta = c(-0.8, 10, 10), AL = 0, bL = 0)

set.seed(3654)
samp=rppi(n, beta = c(-0.8, 10, 10), AL = 0, bL = 0,maxden = 4)
...
# PPI Model from Section 2.3 of Scealy and Wood (2022)
...
data.frame(samp) %>%
  dplyr::rename(u1 = X1, u2 = X2, u3 = X3) %>%
  ggtern(aes(x = u1, y = u2, z = u3)) +
  geom_density_tern(bins = 15, bdl = 0.005, base = "identity") + 
  geom_point(size = 1, alpha = 0.5, shape = "+") +
  ggtitle("Sample Points")
```

# A PPI with Concentration on Centre of One Edge and Some Interior Mass
```{r ppi_egdeconcentration, warning=FALSE, out.width = "0.5\\linewidth"}
ppi_plotdensity(beta = c(0, 0, -0.5), AL = diag(c(0, -5)), bL = c(0, 5))

set.seed(3654)
samp=rppi(n, beta = c(0, 0, -0.5), AL = diag(c(0, -5)), bL = c(0, 5),maxden = 4)
...
data.frame(samp) %>%
  dplyr::rename(u1 = X1, u2 = X2, u3 = X3) %>%
  ggtern(aes(x = u1, y = u2, z = u3)) +
  geom_density_tern(bins = 5, bdl = 0.01, base = "identity") + 
  geom_point(size = 1, alpha = 0.5, shape = "+") +
  ggtitle("Sample Points")
```
