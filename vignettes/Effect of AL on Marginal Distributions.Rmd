...
author: "Kassel Hingee with advice from Janice Scealy"
date: "22/06/2022"
output: html_document
---

The effect of the matrix $A_L$ on marginal distributions can be large, and override the Dirichlet component of the density. This is because the PPI density behaves a bit like a multivariate Gaussian with inverse covariance matrix $A$ (given by $A_L$) with mean $b$. If the mean is far away from the simplex then the PPI distribution does strange things, much like a truncated normal does. (If the mean is in the centre of the simplex then the Dirichlet compenent of the PPI model doesn't matter much - the Dirichlet parameters are not constrained; in fact the full PPI model makes most sense when the mean from $A_L$ is close to the boundary of the simplex).



A large `maxden` required for `rhybrid()` would be symptomatic of peaks near zero occurring at odds to the Dirichlet component of the density.
...
crds <- crds[crds$z>=0, ]
crds$uAu <- qldppi(as.matrix(crds), beta0 = rep(0, p), ALs = ALs, bL = bL)
