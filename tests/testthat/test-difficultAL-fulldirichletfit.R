...
test_that("full ppi estimates are within 3 SE for difficult AL with large maxden, p = 3", {
  set.seed(12735)
  p = 3
  ALs <- exp(rsymmetricmatrix(p-1, -4, 4))
  bL <- rep(0, p-1)
  beta <- c(-0.7, -0.3, 0)
  set.seed(1111) #this seed leads to some ginormous elements for the second diagonal element of ALs
  expect_warning(prop <- rhybrid(20000, p, beta, ALs, bL, 20)$samp3)
  prop %>% as_tibble() %>% tidyr::pivot_longer(everything()) %>% ggplot() + facet_wrap(vars(name)) + geom_freqpoly(aes(x=value))
  ...
  #also expect that the beta are within a fraction of the true values
  expect_equal(est_cppad$est$beta, beta, tolerance = 1E-1)
})
