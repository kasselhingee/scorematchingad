...
test_that("full ppi estimates are mostly within 3 SE for difficult AL with large maxden, p = 5", {
  ...
  suppressMessages(prop <- rppi(1000, beta = beta, AL=ALs, bL=bL, maxden=35)$samp3) #rppi_singly took 1005 seconds, rppi() took 13seconds
  #prop %>% as_tibble() %>% tidyr::pivot_longer(everything()) %>% ggplot() + facet_wrap(vars(name)) + geom_freqpoly(aes(x=value))

  est_cppad <- ppi(prop, ppi_paramvec(bL = bL), trans = "sqrt", bdryweight = "minsq",
                         method = "cppad",
                         acut = 0.01,
                         bdrythreshold = 1E-20,
...
test_that("full ppi estimates are within 3 SE of beta for difficult AL with large maxden, p = 3", {
  ...
  suppressMessages(prop <- rppi(20000, beta=beta, AL=ALs, bL=bL, maxden=20)$samp3)
