...
test_that("fixed beta[p] with minsq weights match true value", {
  ...
  psphere <- pmanifold("sphere")
  pdir <- ptapell(c(0.1,0.1,0.1), c(3,3,3), llname = "dirichlet", psphere, fixedtheta = c(FALSE, FALSE, TRUE))
  smofun <- ptapesmo(c(0.1,0.1,0.1),
                     c(1, 2),
                     pll = pdir,
                     pman = psphere, "minsq", acut = acut) #tape of the score function

  pForward0(pdir, utabl[1,], beta[-3] * 0)
  pForward0(smofun, beta[-3], utabl[1, ])
  ...
  val <- smobj(smofun, beta[-3], utabl)
  val2 <- smobj(smofun, out$par, utabl)
  val3 <- smobj(smofun, beta[-3] * 0, utabl)

  expect_equal(out$par, beta[-3], tolerance = 1E-3, ignore_attr = TRUE)
})
