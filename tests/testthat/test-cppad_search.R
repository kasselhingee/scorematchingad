...
test_that("cppad_search gives similar result to cppad_closed", {
  ...
  set.seed(1) #seems to be some differences between macos, win and ubuntu on following optimisation
  estsearch <- cppad_search(tapes$smdtape, m$theta *0 + 1, m$sample, control = list(tol = 1E-10))
  estclosed <- cppad_closed(tapes$smdtape, m$sample)
  expect_equal(estsearch$est, estclosed$est, tolerance = 2E-3, ignore_attr = "names")
