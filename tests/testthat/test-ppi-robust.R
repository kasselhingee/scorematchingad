...
test_that("Robustness runs for direct and cppad methods", {
  ...
  # estimator1 bL=0
  out <- ppi(m$sample, ppi_paramvec(bL = 0, beta = m$beta), acut=0.1, method = "direct", trans = "sqrt", bdryweight = "minsq", cW = ppi_cW_auto(1E-1, m$sample))
  expect_gt(out$optim$fpevals, 1)
  ...
  # estimator1 bL!=0
  out <- ppi(m$sample, ppi_paramvec(beta = m$beta), acut=0.1, method = "direct", trans = "sqrt", bdryweight = "minsq", cW = ppi_cW_auto(1E-1, m$sample))
  expect_gt(out$optim$fpevals, 1)
  # estimator1 bL!=0 prodsq
  ...
  out <- ppi(m$sample,
             ...
             control = list(tol = 1E-10, maxit = 100, MaxIter = 100),
             cW = ppi_cW_auto(1E-2, m$sample))
  expect_gt(out$optim$fpevals, 1)
})
