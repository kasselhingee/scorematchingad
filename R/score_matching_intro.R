...
#' Score matching was first developed by \insertCite{hyvarinen2005es;textual}{scorecompdir} and was further developed for subsets of Euclidean space \insertCite{hyvarinen2007ex,yu2019ge,yu2020ge,liu2021es}{scorecompdir}, Riemannian manifolds \insertCite{mardia2016sc,mardia2018ne}{scorecompdir},
...
#' In the most general form (Riemannian manifolds with boundary) score matching minimises the weighted Hyvärinen divergence \insertCite{@Equation 7, @scealy2022sc}{scorecompdir} 
#' \deqn{
...
#'  + \eqn{h(z)} is a function, termed the *boundary weight function*, that is zero on the boundary of \eqn{M} \insertCite{@Section 3.2, @scealy2022sc}{scorecompdir}.
...
#'  and the boundary weight function is smooth or of particular forms for specific manifolds \insertCite{@Section 3.2, @scealy2022sc}{scorecompdir},
#'  then minimising the weighted Hyvärinen divergence \eqn{\phi(f, f_0)} is equivalent to minimising the score matching discrepancy \insertCite{@Theorem 1 @scealy2022sc}{scorecompdir}
#' \deqn{
...
#' 
#' We suspect that  \insertCite{@Theorem 1 @scealy2022sc}{scorecompdir} holds more generally for nearly all realistic continuous and piecewise-smooth boundary weight functions, although no proof exists to our knowledge.
#' 
...
#' However, when the family of models is a canonical exponential family then often \eqn{\hat\psi(f_\theta, f_0)} is a quadratic function of \eqn{\theta} \insertCite{mardia2018ne}{scorecompdir} and the minimum has a closed-form solution found by `cppad_closed()`.
#' 
...
#' 
#' # Transformations
#' Hyvärinen divergence (\ref{hyvdiv}) is sensitive to transformations of the manifold.
...
#' Divergences that use the sphere, obtained from the simplex by a square root, have different behaviour to Hyvärinen divergence using a Euclidean space obtained from the simplex using logarithms \insertCite{scealy2024ro}{scorecompdir}.
