...
#' Score matching was first developed by \insertCite{hyvarinen2005es;textual}{scorecompdir} and was further developed for subsets of Euclidean space \insertCite{hyvarinen2007ex,yu2019ge,yu2020ge,liu2019es}{scorecompdir}, Riemannian manifolds \insertCite{mardia2016sc,mardia2018ne}{scorecompdir},
#'  and Riemannian manifolds with boundary \insertCite{scealy2023sc}{scorecompdir}.
#' In this help entry we briefly describe score matching estimation.
#' 
#' @details
#' # Score Matching in General
#' In the most general form (Riemannian manifolds with boundary) score matching minimises the weighted Hyvärinen divergence \insertCite{@Equation 7, @scealy2023sc}{scorecompdir} 
#' \deqn{
...
#'  + \eqn{h(z)} is a function, termed the *boundary weight function*, that is zero on the boundary of \eqn{M} and smooth \insertCite{@Section 3.2, @scealy2023sc}{scorecompdir} or potentially piecewise smooth.
...
#'  and the boundary weight function is smooth or of particular forms for specific manifolds \insertCite{@Section 3.2, @scealy2023sc}{scorecompdir},
#'  then minimising the weighted Hyvärinen divergence \eqn{\phi(f, f_0)} is equivalent to minimising the score matching discrepancy \insertCite{@Theorem 1, @scealy2023sc}{scorecompdir}
#' \deqn{
...
#' 
#' We suspect that  \insertCite{@Theorem 1, @scealy2023sc}{scorecompdir} holds more generally for nearly all realistic continuous and piecewise-smooth boundary weight functions, although no proof exists to our knowledge.
#' 
...
#' However, when the family of models is a canonical exponential family then often \eqn{\hat\psi(f_\theta, f_0)} and the score matching discrepancy function is a quadratic function of \eqn{\theta} \insertCite{mardia2018ne}{scorecompdir} and the minimum has a closed-form solution found by `cppad_closed()`.
#' 
