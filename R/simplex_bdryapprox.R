...
#' @description 
#' Shift compositional measurements by `shiftsize` towards the centre of the simplex.
...
#' @return The measurements `Y` translated `shiftsize` distance towards the simplex centre.
...
#' @description 
#' Split the matrix of observations `Y` into boundary points and interior points using [`simplex_isboundary()`].
...
#' @details Measurements on the boundary of the simplex can lead to degenerate log-likelihood values, which are avoided in score matching by the divergence weight function. However, `CppAD` still computes the log-likelihood and derivatives, and (to KLH's knowledge) is not able to evaluate the value of \eqn{0/0} or similar.
#' However, very good approximations of the score matching objective value can be obtained through Taylor approximations, which requires a centre.
