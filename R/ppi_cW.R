...
#' Elements of \eqn{A_L} and \eqn{b_L} corresponding to components with strong concentrations have non-zero constant tuning exponent, and all other elements have a tuning constant of zero.
...
#' creating weights of infinity for models with concentrations at zero \insertCite{@scealy2022draft}{scorecompdir}.
#' \insertCite{@scealy2022draft}{scorecompdir} avoided these infinite-valued weights by setting some parameters of the PPI model to zero when calculating the weights.
#' \insertCite{@scealy2022draft}{scorecompdir} chose tuning exponents of zero for:
#'  * elements of \eqn{\beta} corresponding to concentrations near zero, which avoids infinte weights
...
#' The remaining tuning exponents were set to a constant.
#' @export
ppi_cW <- function(cW, ...){
  bools <- unlist(list(...))
...
#' @rdname ppi_cW
#' @details The function `ppi_cW_auto()` automatically detects concentrations near zero by fitting a PPI distribution with \eqn{A_L=0} and \eqn{b_L=0} (i.e. a Dirichlet distribution) with the square root transformation and minima-based divergence weight function.
#' @examples
