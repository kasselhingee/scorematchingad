...
#' @param ytape An example measurement value to use for creating the tape. In the natural manifold of the log-likelihood function. `ytape` will be converted to the manifold according to the `toM()` method for `tranobj` before taping. 
...
#' @describeIn buildsmotape Creates a `CppAD` tape of the log-likelihood as a function of values on the provided manifold. The log-likelihood accounts for the change of measure between the natural manifold of the log-likelihood and the manifold with transformation provided by `tranobj`.
#' @return 
